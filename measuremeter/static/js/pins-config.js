...
jQuery(function () {
  ...
  if(pins_len > 0) {
    ...
    var tsvg_obj = document.getElementById("eujspins");
    ...
    for (var i = 0; i < pins_len; i++) {
      if (pins_config.pins[i].shape === "circle") {
        ...
        svg_circle.setAttributeNS(null, "id", "eujspins_" + i);
        tsvg_obj.appendChild(svg_circle);
        eujsAddEvent(i);
      }
        ...
        svg_rect.setAttributeNS(null, "id", "eujspins_" + i);
        tsvg_obj.appendChild(svg_rect);
        eujsAddEvent(i);
      }
    }
...
function eujsAddEvent(id) {
  var obj = jQuery("#eujspins_" + id);
  if(pins_config.pins[id].active === true){
    obj.attr({"cursor": "pointer"});
    obj.hover(function () {
      jQuery("#eujstip").show().html(pins_config.pins[id].hover);
