              ...
              var jsonData = JSON.parse(data);
               ...
               jsonData.forEach(function (item, index) {
                    ...
                    if (item.canton.level == 0)
                    {
                        ...
                        if (id > -1)
                            {
                                statesData.features[id].properties.level = item.development7to7;
                                statesData.features[id].properties.name = item.canton.name;
                                statesData.features[id].properties.comment = "<div align='center'><p>"+item.development7to7 +'%</p><p>' + gettext('Population') + ':<br>' + item.canton.population.toLocaleString('ch-CH') + '<br>' + gettext('Last update') + ':<br>' + item.date+'</div><br><img src="/static/images/graphs_ch/'+ item.canton.code +'_7.png"><br>' + gettext('Development past 2 months.') + '</p>';
                                statesData.features[id].properties.code = item.canton.code + "_7"
                                statesData.features[id].properties.incidence = item.incidence_past7days
                            }
                    }
                        ...
                        if (id > -1)
                            {
                                ...
                                if (item.development7to7 != null)
                                {
                                    statesData.features[id].properties.level = item.development7to7;
                                    statesData.features[id].properties.name = item.canton.name;
                                    statesData.features[id].properties.incidence = item.incidence_past7days;

                                    statesData.features[id].properties.comment = "<div align='center'><p>"+item.development7to7 +'%</p><p>' + gettext('Population') + ':<br>' + item.canton.population.toLocaleString('ch-CH') + '<br>' + gettext('Last update') + ':<br>' + item.date+'</div><br><img src="/static/images/graphs_ch/'+ item.canton.swisstopo_id +'_7.png"><br>' + gettext('Development past 2 months.') + '</p>';
                                    statesData.features[id].properties.code = item.canton.swisstopo_id + "_7"
                                }
                            }
                    }
    ...
    if (info)
    {
        ...
        info.update = function (props) {
            ...
            this._div.innerHTML = '' +  (props ?
                '<div align=center><p><b>' + props.name + '</b></p>' + datestr + commentstr+"Inzidenz 7T:<br> "+props.incidence+'</div>'
        ...
        function onMapClick(e) {
            ...
            popup
                .setLatLng(e.latlng)
                .setContent('<div align=center><p><b>' + e.sourceTarget.feature.properties.name + '</b></p>' + datestr + commentstr+"Inzidenz 7T:<br> "+e.sourceTarget.feature.properties.incidence+'</div>')
                .openOn(map);
        }
        ...
        labels = ['<strong>Tendency</strong>'],
    //    categories = ["<5","<10","<15","<20","<30","<50","<70","<90","<120","<150",">150"];
    //    categories_vals = [4,9,14,19,29,49,69,89,119,121,151];
        categories = [">50",">40",">30",">20",">10",">5",">0",">-5",">-10",">-20",">-30",">-40",">-50","<-50"];
        categories_vals = [49,39,29,19,9,4,0,-4,-9,-19,-29,-39,-49,-51];
