...
var data
    ...
    moment.updateLocale('en', {
     ...
     function FormatPopUp(line, last_line)
     {
        ...
        if (line["KOF_value_before"] > line["KOF_value"])
               {
                tendency = "&#11015;"
               }
               else if (line["KOF_value_before"] == line["KOF_value"])
               {
                tendency = "&#10145;"
               }
               else if (line["KOF_value_before"] < line["KOF_value"])
               {
                tendency = "&#11014;"
                            ...
                            htmlLine = '<p><b>'+ line['canton']['name'] + "</b><br><br>  KOF-Index: "+ line["KOF_value"] + tendency+"<br>KOF index before:"+line["KOF_value_before"] +"<br>"+line["comment"]+"</p>";
    ...
    function prep_annotations(jsonData, annotations_prepare)
    {
            ...
            if (line['date'] != null)
            {
                ...
                let obj = annotations_prepare.find((o, i) => {
                    if (o.value === line['date']) {
                        ...
                        annotations_prepare[i] = { label: codes, value: line['date'], popup: popUp };
                        doesexist = true;
     ...
     function LoadMeasureGraph(startdate, enddate, gr_cantons, gr_measuretypes)
     {
        ...
        annotations_prepare = new Array()
          ...
          var data = $.ajax({
          url: "/measuremeterdata/chstringency/?canton="+gr_cantons,
          dataType: "json",
          async: false
          }).responseText;
