...
def get_vals_by_agegroup(filename, date_str, age_group, date_week, zf, geo):
    try:
        ...
        return cases_vacc, total_vacc, cases_unvacc, total_unvacc
    except:
...
def calc_week(week, geo, age_categories):
    ...
    with urllib.request.urlopen("https://www.covid19.admin.ch/api/data/context") as url:
        ...
        for ac in age_categories:
            cases_vacc[ac], tot_vacc[ac], cases_unvacc[ac], tot_nonvacc[ac] = get_vals_by_agegroup("COVID19Cases_vaccpersons_AKL10_w.csv", "date", ac, date_week ,zf, geo)
        ...
        for ac in age_categories:
            hosp_vacc[ac], tot_vacc[ac], hosp_unvacc[ac], tot_nonvacc[ac] = get_vals_by_agegroup(
        ...
        for ac in age_categories:
            death_vacc[ac], tot_vacc[ac], death_nonvacc[ac], tot_nonvacc[ac] = get_vals_by_agegroup(
...
def create_image(week, geo):
    ...
    print("CSV:")
    print("Week;Category;Age Group;Vacc Num;Unvacc Num;Vacc Incidence;Unvacc Incidence;Ratio;Eff")
    for week_to_load in range (30, (week+1)):
        response = calc_week(week_to_load, geo, age_categories)

        for ac in age_categories:
            print(f"{week_to_load};Cases;{ac};{response['cases_vacc'][ac]};{response['cases_unvacc'][ac]};{response['inz_vacccases'][ac]};{response['inz_nonvacccases'][ac]};{response['rel_cases_str'][ac]};;")
            print(f"{week_to_load};Hosp;{ac};{response['hosp_vacc'][ac]};{response['hosp_unvacc'][ac]};{response['inz_vacchosp'][ac]};{response['inz_nonvacchosp'][ac]};{response['rel_hosp_str'][ac]};{response['eff_hosp_str'][ac]};")
            print(f"{week_to_load};Death;{ac};{response['death_vacc'][ac]};{response['death_nonvacc'][ac]};{response['inz_vaccdeath'][ac]};{response['inz_nonvaccdeath'][ac]};{response['rel_death_str'][ac]};{response['eff_hosp_str'][ac]};")
    ...
    for ac in age_categories:
        html += f'<tr><td>{ac}</td><td style="background-color:#edefee;"></td><td class="right aligned">{"{:10.1f}".format(response["inz_vacccases"][ac])}</td><td class="right aligned">{"{:10.1f}".format(response["inz_nonvacccases"][ac])}</td><td class="right aligned">{response["rel_cases_str"][ac]}</td><td style="background-color:#edefee;"></td><td class="right aligned">{"{:10.1f}".format(response["inz_vacchosp"][ac])}</td><td class="right aligned">{"{:10.1f}".format(response["inz_nonvacchosp"][ac])}</td><td>{response["rel_hosp_str"][ac]}</td><td>{response["eff_hosp_str"][ac]}</td><td style="background-color:#edefee;"></td><td class="right aligned">{"{:10.1f}".format(response["inz_vaccdeath"][ac])}</td><td class="right aligned">{"{:10.1f}".format(response["inz_nonvaccdeath"][ac])}</td><td>{response["rel_death_str"][ac]}</td><td>{response["eff_death_str"][ac]}</td></tr>'
