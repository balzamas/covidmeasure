...
class Command(BaseCommand):
    def handle(self, *args, **options):
      ...
      url = 'https://raw.githubusercontent.com/rsalzer/COVID_19_BAG/master/data/vd.csv'
      ...
      with requests.Session() as s:
        ...
        for canton in CHCanton.objects.filter(level=0, code="vd"):
            ...
            for row in my_list:
                ...
                if row[1].lower() == cantoncode.lower():
                    try:
                        print(row[2])
                        print(row[0])
                        ...
                        if (row[2] is not '' or date_object<(datetime.today()- timedelta(6))):
                            if row[2] is '':
                                    cases_today = 0
                            else:
                                    cases_today = int(row[2]) - old_value
                                    old_value = int(row[2])
